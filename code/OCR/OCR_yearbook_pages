...
image_dir = os.path.join(script_dir, 'yearbook_pages_input')
...
script_dir = os.path.dirname(os.path.abspath(__file__))
output_dir = os.path.join(script_dir, 'yearbook_pages_output')
...
for image_path in image_files:
    ...
    left_proc = preprocess(left)
    left_text = pytesseract.image_to_string(left_proc)
    right_proc = preprocess(right)
    right_text = pytesseract.image_to_string(right_proc)
    
    base_name = os.path.splitext(os.path.basename(image_path))[0]
    
    with open(os.path.join(output_dir, f"processed_{base_name}_left.txt"), 'w', encoding='utf-8') as f:
        f.write(left_text)
    
    with open(os.path.join(output_dir, f"processed_{base_name}_right.txt"), 'w', encoding='utf-8') as f:
        f.write(right_text)